...
function repo_exists() {
  if [ $(s3cmd ls s3://repository.cask.co/centos/6/x86_64/cdap/ | grep ${__maj_min}) ]; then
    return 0
  else
    return 1
  fi
...
function setup_repo_staging() {
  mkdir -p ${STAGE_DIR}/${__maj_min}
  if repo_exists; then
    s3cmd sync s3://repository.cask.co/centos/6/x86_64/cdap/${__maj_min} ${STAGE_DIR}/{__maj_min}
  fi
}

function add_packages_to_repo_staging() {
  mkdir -p ${STAGE_DIR}/{__maj_min}/rpms
  for __rpm in $(ls -1 ${REPO_HOME}/*/target/*.rpm ${REPO_HOME}/target/*.rpm >/dev/null 2>&1); do
...
function sign_packages_in_repo_staging() {
  ...
  for __rpm in $(ls -1 *.rpm); do
    rpm --checksig ${__rpm} | grep -e pgp -e gpg
    __ret=$?
    if [[ ${__ret} ]]; then
...
# This requires a few variables in the environment
# VERSION = CDAP version
# GPG_PASSPHRASE
